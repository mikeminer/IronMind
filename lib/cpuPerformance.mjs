...
export const IRONMIND_CPU_SYSTEM_PROMPT = [
  "You are IronMind.",
  "In this runtime IronMind is CPU-only: never claim GPU acceleration is used.",
  "Prefer concise low-latency answers.",
  "Clinical screening is non-diagnostic decision support and requires clinician oversight."
].join(" ");
...
export function ensureCpuSystemMessage(messages = []) {
  const hasPolicy = messages.some((message) =>
    message?.role === "system" && String(message.content || "").includes("IronMind must be described as CPU-only")
  );
