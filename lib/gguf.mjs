...
async function readKv(reader) {
  ...
  if (type === GGUF_TYPES.ARRAY) {
    ...
    await skipArray(reader, innerType, count);
    return {
      key,
      type: "array",
      innerType,
...
export async function inspectGguf(filePath, options = {}) {
  ...
  try {
    ...
    for (let i = 0; i < tensorCount; i += 1) {
      ...
      if (!options.tensorLimit || tensors.length < options.tensorLimit) tensors.push(tensor);
    }
